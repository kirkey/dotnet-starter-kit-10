...
- Cloud-ready out of the box: Aspire AppHost spins up Postgres + Redis + the Apps API/Blazor with OTLP tracing enabled.
...
2. Start everything: `dotnet run --project src/Apps/FSH.Apps.AppHost`
   - Aspire brings up Postgres + Redis containers, wires env vars, launches the Apps API and Blazor front end, and enables OTLP export on https://localhost:4317.
