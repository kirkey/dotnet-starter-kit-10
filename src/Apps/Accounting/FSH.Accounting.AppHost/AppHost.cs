...
IResourceBuilder<PostgresDatabaseResource> postgres = builder.AddPostgres("postgres").WithDataVolume("fsh-postgres-data").AddDatabase("fsh");

IResourceBuilder<RedisResource> redis = builder.AddRedis("redis").WithDataVolume("fsh-redis-data");

builder.AddProject<Projects.Apps_Api>("apps-api")
    ...
    .WithEnvironment("DatabaseOptions__MigrationsAssembly", "FSH.Accounting.Migrations.PostgreSQL")
    .WaitFor(postgres)
    .WithReference(redis)
    .WithEnvironment("CachingOptions__Redis", redis.Resource.ConnectionStringExpression)
    .WaitFor(redis);

builder.AddProject<Projects.Apps_Blazor>("apps-blazor");
