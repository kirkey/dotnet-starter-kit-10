...
builder.AddProject<Projects.Basic_Api>("apps-api")
    ...
    .WithEnvironment("DatabaseOptions__MigrationsAssembly", "FSH.Basic.Migrations.PostgreSQL")
    .WaitFor(postgres)
    .WithReference(redis)
    .WithEnvironment("CachingOptions__Redis", redis.Resource.ConnectionStringExpression)
    .WaitFor(redis);

builder.AddProject<Projects.Basic_Api>("apps-blazor");
