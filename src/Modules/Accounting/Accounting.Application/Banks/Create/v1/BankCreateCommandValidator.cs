...
public class CreateBankCommandValidator : AbstractValidator<CreateBankCommand>
{
        ...
        [FromKeyedServices("accounting:banks")] IReadRepository<Bank> repository)
    {
        ...
        RuleFor(x => x.BankCode)
            ...
            .MustAsync(async (bankCode, cancellationToken) =>
                (await repository.FirstOrDefaultAsync(new BankByCodeSpec(bankCode), cancellationToken).ConfigureAwait(false)) == null)
            .WithMessage("Bank code '{PropertyValue}' already exists.");
        ...
        RuleFor(x => x.RoutingNumber)
            ...
            .MustAsync(async (routingNumber, cancellationToken) =>
            {
                if (string.IsNullOrWhiteSpace(routingNumber)) return true;
                return (await repository.FirstOrDefaultAsync(new BankByRoutingNumberSpec(routingNumber), cancellationToken).ConfigureAwait(false)) == null;
            }).WithMessage("Routing number '{PropertyValue}' already exists.");
