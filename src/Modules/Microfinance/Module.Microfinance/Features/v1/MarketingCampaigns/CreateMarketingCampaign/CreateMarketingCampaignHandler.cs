...
{
    public async ValueTask<Guid> Handle(CreateMarketingCampaignCommand command, CancellationToken ct)
    {
        var entity = MarketingCampaign.Create(
            command.Name,
            currentUser.GetTenant() ?? "root",
            currentUser.GetUserId(),
            currentUser.Name ?? "System");
        
        context.MarketingCampaigns.Add(entity);
